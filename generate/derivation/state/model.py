...
class SemanticQuantity:
    ...
    ``unit`` may be ``None`` to preserve behavior for unitless extracted quantities.
    Replay can override the operand unit with the ledger key's running unit, matching
    the existing accumulation behavior ("9 more" inherits the anchor unit).
    """
    ...
    def __post_init__(self) -> None:
        ...
        _require_optional_str(self.unit, "SemanticQuantity.unit")
...
class StateKey:
    ...
    ``entity`` remains optional in S2 to preserve current accumulation behavior on
    clauses whose loose leading subject is absent.  Later ADR-0184 phases can tighten
    this once explicit entity binding exists.
    """
    ...
    def __post_init__(self) -> None:
        ...
        _require_optional_str(self.unit, "StateKey.unit")
