...
Specify a math-domain corpus-decomposition mechanism that produces
**`MathReaderRefusalShapeProposal`** records from the audit corpus.
Each proposal:
...
The mechanism is the math-domain analog of cognition's
...
**ADR-0172-impl-W1** — Schema:
- `teaching/math_contemplation_proposal.py` with
  `MathReaderRefusalShapeProposal` dataclass
- Canonical-bytes serialization analogous to W1-A's
  `MathReaderRefusalEvidence`
- Tests: schema invariants, byte-identical determinism

**ADR-0172-impl-W2** — Decomposer:
- `teaching/math_contemplation_decomposition.py::decompose_audit(audit_path) -> tuple[MathReaderRefusalShapeProposal, ...]`
- Naive grouping by `(refusal_reason, missing_operator)` with min-2
  evidence threshold
- Tests: known refusal shapes produce known proposals; same audit →
  byte-identical proposal tuple; rejected-proposal de-duplication

**ADR-0172-impl-W3** — CLI lane:
- `core eval math-contemplation` runs decomposer and emits proposals
  to `teaching/math_proposals/proposals.jsonl`
- Tests: CLI is deterministic; replay-equivalent across runs

**ADR-0172-impl-W4** — Integration:
- Workbench (ADR-0160) renders math proposals alongside cognition
  proposals (this is FOLLOWUPS §4's natural pickup site)
- E2E test: refusal → decomposer → proposal → ratification handler
  → pack mutation → next-audit-pass admits
