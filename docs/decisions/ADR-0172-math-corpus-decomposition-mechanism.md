...
2. **Held-out test set.** A subset of the refusal corpus (e.g. 30%)
   is held out from the evidence pointers used to form the hypothesis.
3. **Inferential application.** The hypothesis is applied to the
   held-out subset: the bridged extraction is computed, run through
   the existing admissibility gates (`_initial_admissible`,
   `roundtrip_admissible`, multi-branch decision rule).
4. **Outcome scoring.**
   - If the bridge admits cases without raising the `wrong` count —
     positive evidence
   - If the bridge produces admissions that disagree with held-out
     ground-truth — negative evidence; proposal is auto-refined or
     dropped before HITL
   - If the bridge produces no admissions but no wrong cases either —
     "neutral" evidence; the bridge is structurally sound but doesn't
     unlock cases on this corpus; proposal goes to HITL with that
     framing
5. **Proposal-with-evidence emission.** The proposal carries both the
   structural claim AND the test result, so HITL review is verification
   of an empirically-tested hypothesis, not adjudication of a raw guess.

The "test and learn" loop is what makes Tier 2 thesis-coherent. The
engine isn't generating a hypothesis and asking the operator to bless
it — it is decoding a structural pattern, testing whether the pattern
holds against unseen cases from its own corpus, and presenting the
operator with the test result as part of the proposal. The operator
adjudicates the *test* + the *interpretation*, not a bare claim.

This is also the wrong=0 safety net for Tier 2: any inferential bridge
that produces a wrong admission in the held-out test set is **rejected
internally** before reaching HITL. The operator only ever sees
proposals whose held-out test preserves wrong=0. Wrong>0 hazards
cannot leak through Tier 2.
