"""Hebrew fluency eval lane runner (Phase 5.2).
...
produces:
  ...
  - Hebrew-script output when `output_language="he"`
  - verb-second word order (predicate first), per
    ``generate.articulation._assemble``
  - surface that contains the expected (subject, predicate, object)
    lexemes drawn from the ``he_core_cognition_v1`` / ``he_logos_micro_v1``
    seed packs

C02–C13 (negation, tense, aspect, quantification, etc.) are not
measured here.  The realizer's tense/aspect/quantifier logic in
``generate/templates.py`` is English-only.  See ``gaps.md`` for the
v2 unblock path: Hebrew morphology + Hebrew rhetorical templates.
...
def _score_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    # is deferred to v2 because the current HE runtime pipeline folds
    # multi-token input to a single lexeme through articulation —
    # documented in gaps.md as a v2 unblock item.
    if not _HEBREW_RE.search(surface):
        failures.append("surface contains no Hebrew script")
    ...
    # accept_surfaces unused at v1; preserved on the case so v2 can use it.
    _ = accept
