...
prose -> comprehend() -> to_total_ordering() -> INDEPENDENT oracle -> result vs gold.
A refusal (unreadable prose, unprojectable, or oracle-refused) is NOT a wrong;
only a committed sort/compare result that disagrees with gold is wrong (must be 0).
"""
...
def main() -> int:
    ...
    if report["wrong"]:
        print("WRONG > 0 — comprehension produced a wrong ordering result:", file=sys.stderr)
